...
#include "database.hpp"
	...
	PersonAdder::PersonAdder(database& data, const char* name) :
		      data(data),
		      name(name)
	{ }

	database& PersonAdder::operator=(double value) {
		  const double mrot=23508;
		  int n=this->data.n;
		  if (value >= mrot){
		  		strcpy(this->data.ptr[n-1].last_name,this->name);
		  		this->data.ptr[n-1].salary=value;
		      return this->data;
	...
	PersonAdder database::operator[](const char* name) {
	...
	bool database::operator==(const database& other) const {
		...
		for (int i = 0; i < n; ++i) {
			if ((string_comparison( ptr[i].last_name, other.ptr[i].last_name ) == false) || (std::abs(ptr[i].salary- other.ptr[i].salary))>eps) return false; 
		}
		return true;
	}
	...
	database database::operator+(const database& other) const {
		int n1=get_size();
...
//        ptr=new person[m];
		...
		std::ostream& operator<<(std::ostream& out, const database& dd) {
			...
			double d_temp;
/*
            std::sort(d.ptr, d.ptr + d.get_size(), [](const person& p1, const person& p2) {
...
*/
	...
	void database::output_with_limits(double a,double b) const {
