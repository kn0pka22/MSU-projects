...
private:
    ...
    const char* name;
public:    
    PersonAdder(database& data, const char* name);
...
public:
	...
	database operator+(const database& other) const;
	
	//database& operator[](const char* name);
    
    	PersonAdder operator[](const char* name); //{
       // return PersonAdder(database, name);
    //}
	...
	void output_with_limits(double a,double b) const;
	
	friend std::ostream& operator<< (std::ostream& out, const database& dd);
	
	~database();
    
    friend class PersonAdder;
};
