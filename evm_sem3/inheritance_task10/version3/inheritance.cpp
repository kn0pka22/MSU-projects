...
#include  <cmath>
 ...
 // входного массива.
        ...
        Array AddConstant::Process(const Array& input) const {
        	std::cout<<"Proces: AddConstant: "<<std::endl;
            Array output(input);
        	size_t s=input.Size();
 ...
 // от массива данных.
    ...
    Array Log::Process(const Array& input) const {
    		std::cout<<"Proces: Log: "<<std::endl;
        size_t s=input.Size();
        Array output(input);
        

        for (size_t i=0;i<s;++i){
        	if ((std::fabs(output[i]) < 1e-14) || (output[i] < 0)) {	
						throw "incorrect argument of the logarithm ";
 					}
    ...
    Array Pipeline::Process(const Array& input) const {
    		std::cout<<"Proces: Pipeline: "<<std::endl;
        Array output(input);
        for (size_t i=0;i<count;++i){
            //ops[i]->Process(output);
            try {
            	output = ops[i]->Process(output);
            }
 						catch(const char* e) {
    ...
    void Pipeline::AddOperator(Operator* Op){
        ...
        count++;
        delete[] ops;
        ops = new Operator*[count + 1];
        for (size_t i=0;i<count;++i) ops[i] = tmp[i];
        delete[] tmp;
        //ops = tmp;
